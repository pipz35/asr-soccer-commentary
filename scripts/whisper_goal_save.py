...
# Save all predictions and references for error analysis
import json as json2
output_data = []
for i in range(len(predictions)):
    output_data.append({"id": i, "ref": references[i], "pred": predictions[i]})
with open("goal_predictions.json", "w") as f:
    json2.dump(output_data, f, indent=2)
print(f"\nSaved {len(output_data)} segments to goal_predictions.json")
